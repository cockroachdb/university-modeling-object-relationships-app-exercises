...
class TestClass:
    ...
    def test_maintenance_foreign_with_action(self, crdb, db="movr_vehicles",
                                  table="maintenance_history",
                                  setup_files=['load_initial_state.sql', 'create_maintenance_history.sql', 'add_foreign_key.sql'],
                                  query_file='add_on_delete_action.sql'):
        ...
        check_foreign_key(crdb,db=db,query_file=query_file,table=table, 
                        column='vehicle_id', ref_table='vehicles', ref_column='id', actions=['ON DELETE CASCADE'])
